...
// The dictionaries used to sit here — 280kB of this file, a third of the main
// bundle's raw bytes, shipped to every reader so each could use the one
// language they picked and ignore four they did not. Strings barely minify, so
// that was real weight on every first load. They are five chunks now, fetched
// on demand; src/i18n/index.js holds the loading rules and what they cost.
const loadLang = () => { try { const l = localStorage.getItem("vantage-lang"); return LANGS.some(x => x.code === l) ? l : "en"; } catch { return "en"; } };
// Asked for at module scope, before React mounts, so the request overlaps the
// rest of boot. A reader coming back to a language that is not English will
// usually have the dictionary in hand by the first paint; on a cold or slow
// connection they see English for a moment first. Holding the paint until it
// arrives would trade that moment for a blank screen whenever the chunk is slow
// or missing, which is the worse of the two.
if (typeof window !== "undefined") loadDict(loadLang());
...
export default function App() {
  ...
  // The dictionary for `lang`, once it is here. Held as {code, table} rather
  // than a bare table so that `t` can tell the right one from the last one: a
  // switch leaves the previous language's table in state until the new one
  // lands, and rendering Spanish keys against the French table would be worse
  // than the English fallback that happens instead.
  const [dict, setDict] = useState(() => peekDict(lang));
  useEffect(() => {
    const have = peekDict(lang);                       // already fetched: no flicker switching back
    if (lang === "en" || have) { setDict(have); return; }
    let live = true;                                   // es → fr → es must not apply es to fr
    loadDict(lang).then(d => { if (live) setDict(d); });
    return () => { live = false; };
  }, [lang]);
  const t = useMemo(() => makeT(lang, dict), [lang, dict]);
